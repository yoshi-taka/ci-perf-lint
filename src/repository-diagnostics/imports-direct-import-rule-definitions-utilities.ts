...
interface UtilityEntryData extends MakeContentOptions {
  depNames: readonly string[];
  meta: typeof dateFnsDirectImportMeta;
  exactSources: readonly string[];
}

const utilityDefs: UtilityEntryData[] = [
  {
    depNames: ["date-fns"],
...
function toUtilityDef(
  dependencyIndex: ReadonlySet<string>,
  data: UtilityEntryData,
...
export function createUtilityImportRuleDefinitions(
  ...
  return [
    ...utilityDefs.map((d) => toUtilityDef(dependencyIndex, d)),
    {
      enabled: dependencyIndexHasEffectDependency(dependencyIndex),
      meta: effectDirectImportMeta,
      exactSources: ["effect"],
      matches: (source) => regexMatcher("^@effect/[^/]+$")(source),
      content: makeContent({
