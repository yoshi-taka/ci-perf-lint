...
describe("parser guardrails", () => {
  test("rejects oversized workflow source", () => {
    expect(() => parseWorkflow("/repo/.github/workflows/ci.yml", "/repo", "a".repeat(5_000_001))).toThrow(
      "Workflow source too large",
    );
  });

  test("rejects oversized platform sources", () => {
    expect(() => parsePipeline("/repo/.buildkite/pipeline.yml", "/repo", "a".repeat(5_000_001))).toThrow(
      "Pipeline source too large",
    );
    expect(() => parseGitlabCi("/repo/.gitlab-ci.yml", "/repo", "a".repeat(5_000_001))).toThrow(
      "GitLab CI source too large",
    );
    expect(() => parseCircleCi("/repo/.circleci/config.yml", "/repo", "a".repeat(5_000_001))).toThrow(
      "CircleCI source too large",
    );
  });
});
